...
function Proyect({title,description,linkGit,linkLive,open,id}){
    ...
    return(
        <div className={isLight ? 'containerProyect' : 'containerProyect backgroundContainerProyect'} >
            ...
            <p className={isLight ? 'descriptionProyect' : 'descriptionProyect textProyectColor'}>{description}</p>
            <div className="linksProyect">
                <div className="buttonProyect buttonMore" onClick={open} id={id}>
                    {text[isSpa].proyects.seeInfo}          
                </div>
                <a href={linkGit} target='_blank' className="buttonProyect link buttonCode">
                    {text[isSpa].proyects.seeCode}             
                </a>
                <a href={linkLive} target='_blank' className="buttonProyect link buttonLiveView">
